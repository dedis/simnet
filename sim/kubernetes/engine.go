...
func (kd *kubeEngine) writeCertificates(pod string, certs sim.Certificates) error {
	...
	// by definition as the app container wouldn't have started if it has
	// failed.
	files := map[string]string{
		certs.Cert: "/etc/openvpn/pki/issued/client1.crt",
		certs.Key:  "/etc/openvpn/pki/private/client1.key",
		certs.CA:   "/etc/openvpn/pki/ca.crt",
	}
